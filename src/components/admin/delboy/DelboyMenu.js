...
import AddDelboyModal from "./AddDelboyModal";
import EditDelboyModal from "./EditDelboyModal";

const DelboyMenu = (props) => {
  const { dispatch } = useContext(DelboyContext);

  return (
    <Fragment>
      <div className="col-span-1 flex items-center">
        <div className="flex flex-col space-y-4 md:flex-row md:justify-between md:items-center w-full">
          {/* It's open the add Delboy modal */}
          <div
            style={{ background: "#303031" }}
            onClick={(e) =>
              dispatch({ type: "addDelboyModal", payload: true })
            }
